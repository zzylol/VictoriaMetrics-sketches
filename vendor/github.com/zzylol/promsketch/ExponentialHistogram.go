...
func ExpoInitCountCS(k int64, time_window_size int64) (eh *ExpoHistogramCS) {
	...
	eh.seed1 = make([]uint32, CS_ROW_NO_Univ)
	eh.seed2 = make([]uint32, CS_ROW_NO_Univ)
...
func (eh *ExpoHistogramCS) Update(time int64, key string, value float64) {
	...
	tmp, err := NewCountSketch(CS_ROW_NO_Univ, CS_COL_NO_Univ, eh.seed1, eh.seed2)
	if err != nil {
...
func (eh *ExpoHistogramCS) GetMemory() float64 {
	return float64(eh.s_count) * (float64(CS_COL_NO_Univ) * float64(CS_ROW_NO_Univ) * 8) / 1024
}
...
func (eh *ExpoHistogramCS) QueryIntervalMergeCount(t1, t2 int64) CountSketch {
	...
	// fmt.Println("to_bucket =", to_bucket)
	if eh.s_count > 1 {
		merged_bucket, err := NewCountSketch(CS_ROW_NO_Univ, CS_COL_NO_Univ, eh.seed1, eh.seed2)
		if err != nil {
			fmt.Println("[Error] failed to allocate memory for new bucket of CountSketch...")
		}
		for i := from_bucket; i <= to_bucket; i++ {
...
func ExpoInitUniv(k int64, time_window_size int64) (ehu *ExpoHistogramUniv) {
	...
	ehu.putch = make(chan int64, 40)
	ehu.done = make(chan struct{}, 40)

	ehu.cs_seed1 = make([]uint32, CS_ROW_NO_Univ)
	ehu.cs_seed2 = make([]uint32, CS_ROW_NO_Univ)
	rand.Seed(time.Now().UnixNano())
	for i := 0; i < CS_ROW_NO_Univ; i++ {
	...
	for i := uint32(0); i < UnivPoolCAP; i++ {
		ehu.univPool.pool[i], _ = NewUnivSketch(TOPK_SIZE, CS_ROW_NO_Univ, CS_COL_NO_Univ, CS_LVLS, ehu.cs_seed1, ehu.cs_seed2, ehu.seed3, int64(i))
	}
...
func (ehu *ExpoHistogramUniv) GetUnivSketch() (UnivSketch, error) {
	...
	if ehu.univPool.size == ehu.univPool.max_size {
		tmp, err := NewUnivSketch(TOPK_SIZE, CS_ROW_NO_Univ, CS_COL_NO_Univ, CS_LVLS, ehu.cs_seed1, ehu.cs_seed2, ehu.seed3, int64(ehu.univPool.max_size)) // New && init UnivMon
		if err != nil {
	...
	if !ehu.univPool.bm.Contains(0) {
		...
		if !flag {
			return UnivSketch{}, errors.New("univ sketch finding failed")
		}
	}
...
func (ehu *ExpoHistogramUniv) Update(time_ int64, fvalue float64) {
	...
	for i := ehu.s_count - 2; i >= 0; i-- {
		if ehu.univs[i].bucketsize == ehu.univs[i+1].bucketsize {
			same_size_bucket += 1
		} else {
			if float64(same_size_bucket) >= float64(ehu.k)/2.0+2 {
				...
				ehu.univs[i+1].bucketsize += ehu.univs[i+2].bucketsize
			...
			same_size_bucket = 1
			if ehu.univs[i+1].bucketsize == ehu.univs[i].bucketsize {
				same_size_bucket += 1
			}
		}
	}
	...
	if float64(same_size_bucket) >= float64(ehu.k)/2.0+2 {
		...
		ehu.univs[0].bucketsize += ehu.univs[1].bucketsize
...
func (eh *ExpoHistogramUniv) Cover(mint, maxt int64) bool {
	...
	return (eh.univs[0].min_time <= mint) // && eh.univs[eh.s_count-1].max_time >= maxt)
}
...
func (ehu *ExpoHistogramUniv) print_buckets() {
	...
	fmt.Println("k =", ehu.k)
	for i := 0; i < ehu.s_count; i++ {
		fmt.Println(i, ehu.univs[i].bucketsize, ehu.univs[i].min_time, ehu.univs[i].max_time)
	}
}
...
func (eh *ExpoHistogramUniv) GetMemory() float64 {
	var total_mem float64 = 0
	for i := 0; i < eh.s_count; i++ {
		total_mem += eh.univs[i].GetMemoryKB()
	}
	return total_mem
}

func (ehu *ExpoHistogramUniv) QueryIntervalMergeUniv(t1, t2 int64) (univ UnivSketch, err error) {
	var from_bucket, to_bucket int = 0, 0
	for i := 0; i < ehu.s_count; i++ {
		...
		/* // Old version
		curdiff1 := AbsInt64(t1 - ehkll.min_time[i])
		curdiff2 := AbsInt64(t2 - ehkll.max_time[i])
		if curdiff1 < diff1 {
			diff1 = curdiff1
			from_bucket = i
		}
	...
	// fmt.Println("to_bucket =", to_bucket)
	if ehu.s_count > 1 {
		merged_univ, _ := NewUnivSketch(TOPK_SIZE, CS_ROW_NO_Univ, CS_COL_NO_Univ, CS_LVLS, ehu.cs_seed1, ehu.cs_seed2, ehu.seed3, -1)
		for i := from_bucket; i <= to_bucket; i++ {
			merged_univ.MergeWith(ehu.univs[i])
		}

		return merged_univ, nil
	} else if ehu.s_count == 1 {
		return ehu.univs[0], nil
	} else {
		return UnivSketch{}, errors.New("not found univ bucket")
	}
