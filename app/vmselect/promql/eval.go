...
func AdjustStartEnd(start, end, step int64) (int64, int64) {
	if *disableCache {
		...
		// See https://github.com/zzylol/VictoriaMetrics-sketches/issues/563
		return start, end
	}
	points := (end-start)/step + 1
...
func execBinaryOpArgs(qt *querytracer.Tracer, ec *EvalConfig, exprFirst, exprSecond metricsql.Expr, be *metricsql.BinaryOpExpr) ([]*timeseries, []*timeseries, error) {
	if !canPushdownCommonFilters(be) {
		...
		// See https://github.com/zzylol/VictoriaMetrics-sketches/issues/2886
	...
	if len(tssFirst) == 0 && !strings.EqualFold(be.Op, "or") {
		...
		// See https://github.com/zzylol/VictoriaMetrics-sketches/issues/3349
		return nil, nil, nil
	}
...
func evalRollupFuncWithoutAt(qt *querytracer.Tracer, ec *EvalConfig, funcName string, rf rollupFunc,
	...
	if re.Offset != nil {
		...
		// See also https://github.com/zzylol/VictoriaMetrics-sketches/issues/976
	}
	if funcName == "rollup_candlestick" {
		// Automatically apply `offset -step` to `rollup_candlestick` function
		// in order to obtain expected OHLC results.
		// See https://github.com/zzylol/VictoriaMetrics-sketches/issues/309#issuecomment-582113462
		step := ecNew.Step
		ecNew = copyEvalConfig(ecNew)
		ecNew.Start += step
		ecNew.End += step
...
// This follows Prometheus logic - see https://github.com/zzylol/VictoriaMetrics-sketches/issues/2130
...
func evalRollupSketchCache(qt *querytracer.Tracer, funcName string, keepMetricNames bool, srs *vmsketch.SketchResults, rcs []*rollupConfig,
	...
	err := srs.RunParallel(qt, func(sr *vmsketch.SketchResult, workerID uint) error {
		for _, rc := range rcs {
			...
			var ts timeseries
			samplesScanned := doRollupForTimeseriesSketch(funcName, keepMetricNames, rc, &ts, sr, sharedTimestamps)
			samplesScannedTotal.Add(samplesScanned)
			seriesByWorkerID[workerID].tss = append(seriesByWorkerID[workerID].tss, &ts)
		}
		return nil
...
func doRollupForTimeseriesSketch(funcName string, keepMetricNames bool, rc *rollupConfig, tsDst *timeseries, sr *vmsketch.SketchResult, sharedTimestamps []int64) uint64 {
	...
	var samplesScanned uint64
	tsDst.Values, samplesScanned = rc.DoSketch(tsDst.Values[:0], sr)
	tsDst.Timestamps = sharedTimestamps
	tsDst.denyReuse = true
	return samplesScanned
}
